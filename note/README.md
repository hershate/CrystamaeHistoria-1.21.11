...
- [0.18.2](release/0.18.2.md)（当前）：补丁版——姿态克隆器 onPoseClone 补 ignoreCancelled（r71 全景矩阵发现、r72 消息哨兵双对照实证）：被否决交互不再误执行克隆/消耗；无数据格式变更，旧存档完全兼容。
- [0.18.1](release/0.18.1.md)：稳定性/正确性修复版——**审计第 34-66 轮收口**：7 项修复（工作台配方快照缺失 42 处/收纳袋 GUI 数据丢失/名称前缀叠加/空池统计污染/**迁移级 FLASH 粒子崩溃（祭坛提取与镀金器吸取全中）**/命令防御解耦/注释对齐）；33 覆盖面验证 + 24 项一键回归套件全绿 + 产物一致性 + 旧存档双读复验；无数据格式变更，旧存档完全兼容。
- [0.18.0](release/0.18.0.md)：第十七轮性能优化版——**驻留
  条目每 tick 重复解析域（r76-78）**：晶簇驻留条目（存活至破坏）的
  Block/粒子 Location 按条目缓存 + 镀金 DustOptions 静态化——成熟
  晶簇每 tick 体 1.61x（卫生级，粒子语义不变）；r77+r78 连续判定
  零发现收口；无数据格式变更，旧存档完全兼容。
- [0.17.0](release/0.17.0.md)：第十六轮性能优化版——**每 tick
  派发任务折叠域（r73-75）**：FloatingHeadAnimation（每工作面板一个
  period=1 长期任务）折叠进 process() 同节拍驱动——~2.5x 噪声级 +
  全库唯一长期每 tick 任务类消除（含卸载区块空转）；r74+r75 连续判定
  零发现收口；无数据格式变更，旧存档完全兼容。
- [0.16.0](release/0.16.0.md)：第十五轮性能优化版——**方块写入
