...
- [0.18.1](release/0.18.1.md)（当前）：稳定性/正确性修复版——**审计第 34-66 轮收口**：7 项修复（工作台配方快照缺失 42 处/收纳袋 GUI 数据丢失/名称前缀叠加/空池统计污染/**迁移级 FLASH 粒子崩溃（祭坛提取与镀金器吸取全中）**/命令防御解耦/注释对齐）；33 覆盖面验证 + 24 项一键回归套件全绿 + 产物一致性 + 旧存档双读复验；无数据格式变更，旧存档完全兼容。
